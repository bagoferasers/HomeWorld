...
{
    return saturate(length(irradiance + FLT_EPS) * _ExposureOcclusion);
}
...
half3 Irradiance_SphericalHarmonicsUnity (half3 normal, float3 worldPos)
{
...
//        o_gi.indirect.diffuse = ShadeSHPerPixel(normalWorld, data.ambient, data.worldPos);
    	...
    	o_gi.indirect.diffuse = Irradiance_SphericalHarmonicsUnity(normalWorld, data.worldPos);
    #endif
        ...
        half3 bakedColor = DecodeLightmap(bakedColorTex);
            ...
            // Bakery's MonoSH mode replaces the regular directional lightmap
            #if defined(_BAKERY_MONOSH)
                o_gi.indirect.diffuse += DecodeMonoSHLightmap (bakedColor, data.lightmapUV.xy, normalWorld, o_gi.light);
                irradianceForAO = o_gi.indirect.diffuse;

                #if defined(LIGHTMAP_SHADOW_MIXING) && !defined(SHADOWS_SHADOWMASK) && defined(SHADOWS_SCREEN)
                    o_gi.indirect.diffuse = SubtractMainLightWithRealtimeAttenuationFromLightmap (irradiance, shading.attenuation, bakedColorTex, normalWorld);
                #endif
            #else
                ...
                // bakery rnm mode
                o_gi.indirect.diffuse = DecodeRNMLightmap(bakedColor, data.lightmapUV.xy, tangentNormal, shading.tangentToWorld, o_gi.light);
                #endif

                #if defined(_BAKERY_SH)
                // bakery sh mode
                o_gi.indirect.diffuse = DecodeSHLightmap(bakedColor, data.lightmapUV.xy, normalWorld, o_gi.light);
                #endif

                irradianceForAO = o_gi.indirect.diffuse;

                #if defined(LIGHTMAP_SHADOW_MIXING) && !defined(SHADOWS_SHADOWMASK) && defined(SHADOWS_SCREEN)
                    irradiance = SubtractMainLightWithRealtimeAttenuationFromLightmap(irradiance, shading.attenuation, bakedColorTex, normalWorld);
                #endif
